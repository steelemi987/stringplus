...
int opu(va_list args, ops* op, char** src, int base) {
  ...
  char* strlim;
  check_buffer(src, op);
  if (s21_atoul(*src, NULL, base) ||
      *(*src + s21_strspn(*src, "\n\t \f\x0B\r+")) == '0') {
    if (!op->supr) {
      if (s21_atoul(*src, NULL, base) || **src == '0') res++;
      if (op->len == 1)
        *(short unsigned*)va_arg(args, void*) = s21_atoul(*src, &strlim, base);
      else if (op->len == 2)
...
int opf(va_list args, ops* op, char** src) {
  ...
  char* strlim;
  check_buffer(src, op);
  if (fabs(s21_atod(*src, NULL)) >= 1e-100 || isnan(s21_atod(*src, NULL)) ||
      *(*src + s21_strspn(*src, "\n\t \f\x0B\r+-")) == '0') {
    if (!op->supr) {
      res++;
      if (op->len == 3)
...
int opp(va_list args, ops* op, char** src) {
  int res = 0;
  char* strlim;
  if (s21_atoul(*src, NULL, 16) ||
      *(*src + s21_strspn(*src, "\n\t \f\x0B\r+-")) == '0') {
    check_buffer(src, op);
    if (!op->supr) {
      if (s21_atoul(*src, NULL, 16) || **src == '0') res++;
...
int s21_sscanf(const char* str, const char* format, ...) {
  ...
  char* tmp = malloc((s21_strlen(str) + 1) * sizeof(char) + 1);
  if (!tmp) {
    printf("ERROR");
  } else {
